...
target_fields = ['Healthcare Organization Name:','Provider Name:','NPI #:','Other(s)','Location Address:','City, State, Zip(1)','Phone Number:','Secure Fax Number":','Patient ID/MRN:','Phone Number (required):','First Name:','Last Name:','Language Preference (optional):','DOB (mm/dd/yyyy):','Shipping Address:','Billing Address:','City, State, Zip:(2)','City, State, Zip:(3)','Policyholder Name:','Policyholder DOB:','Primary Insurance Carrier:','Claims Submission Address:','Subscriber ID/Policy Number:','Group Number:','Plan:','Prior-Authorization Code (if available):','Date:']
...
for handwritten_text in handwritten_texts:
	...
	for target_field in target_bounding_boxes:
		...
		if polygon_ptext.contains(polygon_htext) == True:
			field_mappings[handwritten_text] = target_field
			break
