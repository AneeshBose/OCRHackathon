	...
	for index,img in enumerate(images):
		...
		for index, line in enumerate(resp["analyzeResult"]["readResults"][0]["lines"]):
			if ('requisition' in line['text'].lower()) or ('demographic' in line['text'].lower()):
				req_flag = 1
			if 'Zip'.lower() in line['text'].lower() and line['boundingBox'][0] < 120:
				csz_counter += 1
			...
			elif 'other' in line['text'].lower() and line['boundingBox'][1] < 650:

				bounding_box_points = calculate_req_bounding_box(line['boundingBox'], req_target_offsets['Other(s)'])
				target_bounding_boxes['Other(s)'] = [(bounding_box_points[i], bounding_box_points[i + 1]) for i in
												  range(0, len(bounding_box_points), 2)]
		...
		for key_field in field_mappings:
			...
			if 'City, State, Zip' in key_field:
				key_field_index = row.lower().find('Zip'.lower())
				key_field_len = len(key_field)
				str_key_field = row[key_field_index+3:]
				key_field_prep = str_key_field
				#key_field_prep = row.replace(str_key_field, '',1)
			elif 'Other(s)' in key_field:
				key_field_prep = row.split(')')[1]
		...
		if 'City, State, Zip2' in field_mappings:
			...
			key_field_index_city = left_text.lower().find('city')
			key_field_index_zip = left_text.lower().find('zip') + 2


			first_field = left_text[:key_field_index_city]
			second_field = left_text[key_field_index_zip + 1:]
